...
<div class="context-list">
  ...
  <div><div>Обще-яндексовые решения</div><span>С которыми хотелось интегрироваться</span></div>
</div>
...
<StickerBoard class="context-sticker-board" seed="format-quorum-context-9" :gap="4">
  ...
  <Sticker v-click><b>66 корнер-кейсов</b><span>отлиты в регрес-тесты</span></Sticker>
  <Sticker v-click><b>2 итерации</b><span>переформатирования всей кодовой базы</span></Sticker>
  <Sticker v-click><b>git clone llvm</b><span>пропатченный clang-format</span></Sticker>
...
<div class="workflow-needs">
  <div v-click><b>Огромное количество корнер-кейсов</b><span>которые обнаруживаются только на раеально коде.</span></div>
  <div v-click><b>Регрессии читаемости</b><span>дораотка конфига приводила к ухудшению ранее сформированного стиля</span></div>
...
<!--
Shadow config переиспользует тот же бинарник, но имеет отдельный конфиг и ведёт себя как квази-версия.
...
# Приложение: языки и formatter'ы<MarkerX color="#888888" title="справка" />

| Язык | Formatter'ы | Config | Версии |
| --- | --- | :---: | :---: |
| <span class="mono-text">C++</span> | clang-format | ✓ | ✓ |
| Python | Ruff, Black | ✓ | ✓ |
| Rust | rustfmt | ✓ | ✓ |
| JavaScript, TypeScript, JSON, CSS, HTML, Markdown, YAML | Prettier | ✓ | ✓ |
| Shell | shfmt | — | ✓ |
| TOML | Taplo | ✓ | ✓ |
| Java | google-java-format | — | ✓ |

<!--
Полный реестр: backend/formatter_registry.py. Всего 13 языков и 14 встроенных formatter entries.
-->
...
# Приложение: инвентаризация функций<MarkerX color="#888888" title="справка" />

::left::

- форматирование и line diff;
- shareable URLs до теста;
- CRUD и mute тестов;
- локальные drafts;
- config lint и history;
- what-if / Check impact;
- version matrix;
- shadow configs.

::right::

- установка произвольной версии;
- стратегии pip, npm, URL, JAR, rustup;
- пользовательские formatter'ы;
- загрузка и скачивание бинарника;
- открытый HTTP API;
- persistent stores;
- read-only production mode;
- agent-facing API skills.

<div class="callout danger">Загрузка бинарников выполняет чужой код: в production она отключена.</div>

<!--
Источники: README, backend/main.py, versions.py, custom_formatter_store.py и два API skill в .claude/skills.
Uploads включаются только ALLOW_BINARY_UPLOAD=1; publishing управляется PUBLISH_ENABLED.
-->
...
# Приложение: проверенная временная шкала<MarkerX color="#888888" title="источники" />

<div class="source-timeline">
  <div><b>3–16 июня</b><span>первый config PR</span><small>PR 13704587</small></div>
  <div><b>16–18 июня</b><span>массовое форматирование</span><small>PR 13935022</small></div>
  <div><b>23 июня</b><span>tests, drafts, what-if, matrix</span><small>git history</small></div>
  <div><b>8 июля</b><span>попытка расширить общий clang</span><small>PR 14318410</small></div>
  <div><b>21–27 июля</b><span>patched binary + CI</span><small>4 merged PR</small></div>
  <div><b>5 августа</b><span>production в read-only</span><small>git history</small></div>
</div>

<p class="muted center-text">Внутренние обсуждения обезличены; числа сверены по исходным PR, тикетам и коду.</p>

<!--
Публичная история продукта: github.com/alchemmist/format-quorum, PR 1, 3, 4, 8, 12, 13, 14, 16 и 19.
Внутренние опорные тикеты: LOGS-5799, LOGS-5800, LOGS-5966 и DUTYLOGS-4106.
-->
